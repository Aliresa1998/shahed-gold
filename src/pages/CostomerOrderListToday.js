...
const CostomerOrderList = (props) => {
  ...
  const handleReadData = async () => {
    setLoading(true);
    const response = await controller.getCustomerOrderListToday(
      page,
      ...
      {mode == "detailData" ? (
        ...
        <>
          ...
          >
            <Card
              title={
                <Row justify="space-between" style={{ margin: "0% 2%" }}>
                  <Col>لیست سفارشات امروز</Col>
                  <div>
                    <ReloadOutlined
            ...
            >
              {loading ? (
                <Row justify="center" style={{ minHeight: "80px" }}>
                  <Spin />
                </Row>
              ) : (
                <>
